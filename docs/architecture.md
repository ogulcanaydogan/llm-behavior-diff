...
- `report` renders table/json/html/markdown/csv/ndjson/junit views.
  ...
  - optional direct connector dispatch (`--export-connector http|s3|bigquery|snowflake`) posts rendered content externally.
  - `bigquery` and `snowflake` connectors are NDJSON-only and fail-fast on insert errors.
