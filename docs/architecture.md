...
- `report` renders table/json/html/markdown/csv/ndjson/junit views.
  ...
  - optional direct connector dispatch (`--export-connector http|s3|bigquery`) posts rendered content externally.
  - `bigquery` connector is NDJSON-only and fail-fast on insert errors.
