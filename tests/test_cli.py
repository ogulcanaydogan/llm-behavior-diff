...
def test_report_postgres_export_connector_success_with_env_password(
    tmp_path: Path, monkeypatch
) -> None:
    report_path = tmp_path / "report_postgres_export.json"
    ndjson_path = tmp_path / "report_postgres_export.ndjson"
    captured: dict[str, object] = {}
    attempts = {"count": 0}
    backoffs: list[float] = []
    ...
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_postgres_export",
        total_tests=1,
        total_diffs=1,
        regressions=1,
        improvements=0,
    ...
    class FakeCursor:
        def executemany(self, query: str, rows: list[dict[str, object]]) -> None:
            attempts["count"] += 1
            if attempts["count"] == 1:
                raise TimeoutError("postgres connection timed out")
            captured["query"] = query
            captured["rows"] = rows
    ...
    def fake_create_postgres_connection(**kwargs):
        captured["connect_kwargs"] = kwargs
        return FakeConnection()

    monkeypatch.setenv("LLM_DIFF_EXPORT_PG_PASSWORD", "pg-secret")
    monkeypatch.setattr("llm_behavior_diff.cli._sleep_export_retry", backoffs.append)
    monkeypatch.setattr(
        "llm_behavior_diff.cli._create_postgres_connection", fake_create_postgres_connection
    )
    ...
    connect_kwargs = captured["connect_kwargs"]
    assert isinstance(connect_kwargs, dict)
    assert connect_kwargs["host"] == "postgres.example.com"
    assert connect_kwargs["port"] == 5432
    assert connect_kwargs["database"] == "analytics"
    assert connect_kwargs["user"] == "svc_llm_diff"
    assert connect_kwargs["password"] == "pg-secret"
    assert connect_kwargs["sslmode"] == "require"
    assert connect_kwargs["timeout_seconds"] == 6.0
...
def test_report_postgres_export_connector_requires_fields(tmp_path: Path, monkeypatch) -> None:
    report_path = tmp_path / "report_postgres_export_missing_fields.json"
    ndjson_path = tmp_path / "report_postgres_export_missing_fields.ndjson"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_postgres_export",
    ...
    report_path.write_text(json.dumps(report.model_dump(mode="json"), indent=2), encoding="utf-8")
    monkeypatch.delenv("LLM_DIFF_EXPORT_PG_PASSWORD", raising=False)
    ...
    assert missing_host.exit_code == 1
    assert "export_pg_host is required" in missing_host.output
    ...
    assert missing_password.exit_code == 1
    assert "PostgreSQL password is required" in missing_password.output
    ...
    assert invalid_port.exit_code == 1
    assert "export_pg_port must be > 0" in invalid_port.output


def test_report_postgres_export_connector_rejects_non_ndjson_format(tmp_path: Path) -> None:
    report_path = tmp_path / "report_postgres_export_non_ndjson.json"
    csv_path = tmp_path / "report_postgres_export_non_ndjson.csv"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_postgres_export",
...
def test_report_mssql_export_connector_success_with_env_password(
    tmp_path: Path, monkeypatch
) -> None:
    report_path = tmp_path / "report_mssql_export.json"
    ndjson_path = tmp_path / "report_mssql_export.ndjson"
    captured: dict[str, object] = {}
    attempts = {"count": 0}
    backoffs: list[float] = []
    ...
    class FakeCursor:
        def executemany(self, query: str, rows: list[tuple[object, ...]]) -> None:
            attempts["count"] += 1
            if attempts["count"] == 1:
                raise TimeoutError("mssql connection timed out")
            captured["query"] = query
            captured["rows"] = rows
    ...
    def fake_create_mssql_connection(**kwargs):
        captured["connect_kwargs"] = kwargs
        return FakeConnection()

    monkeypatch.setenv("LLM_DIFF_EXPORT_MS_PASSWORD", "ms-secret")
    monkeypatch.setattr("llm_behavior_diff.cli._sleep_export_retry", backoffs.append)
    monkeypatch.setattr(
        "llm_behavior_diff.cli._create_mssql_connection", fake_create_mssql_connection
    )
    ...
    connect_kwargs = captured["connect_kwargs"]
    assert isinstance(connect_kwargs, dict)
    assert connect_kwargs["host"] == "mssql.example.com"
    assert connect_kwargs["port"] == 1433
    assert connect_kwargs["database"] == "analytics"
    assert connect_kwargs["user"] == "svc_llm_diff"
    assert connect_kwargs["password"] == "ms-secret"
    assert connect_kwargs["timeout_seconds"] == 6.0
...
def test_report_mssql_export_connector_requires_fields(tmp_path: Path, monkeypatch) -> None:
    report_path = tmp_path / "report_mssql_export_missing_fields.json"
    ndjson_path = tmp_path / "report_mssql_export_missing_fields.ndjson"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_mssql_export",
    ...
    report_path.write_text(json.dumps(report.model_dump(mode="json"), indent=2), encoding="utf-8")
    monkeypatch.delenv("LLM_DIFF_EXPORT_MS_PASSWORD", raising=False)
    ...
    assert missing_host.exit_code == 1
    assert "export_ms_host is required" in missing_host.output
    ...
    assert missing_password.exit_code == 1
    assert "MSSQL password is required" in missing_password.output
    ...
    assert invalid_port.exit_code == 1
    assert "export_ms_port must be > 0" in invalid_port.output


def test_report_mssql_export_connector_rejects_non_ndjson_format(tmp_path: Path) -> None:
    report_path = tmp_path / "report_mssql_export_non_ndjson.json"
    csv_path = tmp_path / "report_mssql_export_non_ndjson.csv"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_mssql_export",
...
def test_report_oracle_export_connector_success_with_env_password(
    tmp_path: Path, monkeypatch
) -> None:
    report_path = tmp_path / "report_oracle_export.json"
    ndjson_path = tmp_path / "report_oracle_export.ndjson"
    captured: dict[str, object] = {}
    attempts = {"count": 0}
    backoffs: list[float] = []
    ...
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_oracle_export",
        total_tests=1,
        total_diffs=1,
        regressions=1,
        improvements=0,
    ...
    class FakeCursor:
        def executemany(self, query: str, rows: list[tuple[object, ...]]) -> None:
            attempts["count"] += 1
            if attempts["count"] == 1:
                raise TimeoutError("oracle connection timed out")
            captured["query"] = query
            captured["rows"] = rows
    ...
    def fake_create_oracle_connection(**kwargs):
        captured["connect_kwargs"] = kwargs
        return FakeConnection()

    monkeypatch.setenv("LLM_DIFF_EXPORT_OR_PASSWORD", "or-secret")
    monkeypatch.setattr("llm_behavior_diff.cli._sleep_export_retry", backoffs.append)
    monkeypatch.setattr(
        "llm_behavior_diff.cli._create_oracle_connection", fake_create_oracle_connection
    )
    ...
    connect_kwargs = captured["connect_kwargs"]
    assert isinstance(connect_kwargs, dict)
    assert connect_kwargs["host"] == "oracle.example.com"
    assert connect_kwargs["port"] == 1521
    assert connect_kwargs["service_name"] == "ORCLPDB1"
    assert connect_kwargs["user"] == "svc_llm_diff"
    assert connect_kwargs["password"] == "or-secret"
    assert connect_kwargs["timeout_seconds"] == 6.0
...
def test_report_oracle_export_connector_requires_fields(tmp_path: Path, monkeypatch) -> None:
    report_path = tmp_path / "report_oracle_export_missing_fields.json"
    ndjson_path = tmp_path / "report_oracle_export_missing_fields.ndjson"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_oracle_export",
    ...
    report_path.write_text(json.dumps(report.model_dump(mode="json"), indent=2), encoding="utf-8")
    monkeypatch.delenv("LLM_DIFF_EXPORT_OR_PASSWORD", raising=False)
    ...
    assert missing_host.exit_code == 1
    assert "export_or_host is required" in missing_host.output
    ...
    assert missing_password.exit_code == 1
    assert "Oracle password is required" in missing_password.output
    ...
    assert invalid_port.exit_code == 1
    assert "export_or_port must be > 0" in invalid_port.output


def test_report_oracle_export_connector_rejects_non_ndjson_format(tmp_path: Path) -> None:
    report_path = tmp_path / "report_oracle_export_non_ndjson.json"
    csv_path = tmp_path / "report_oracle_export_non_ndjson.csv"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_oracle_export",
...
def test_report_mysql_export_connector_success_with_env_password(
    tmp_path: Path, monkeypatch
) -> None:
    report_path = tmp_path / "report_mysql_export.json"
    ndjson_path = tmp_path / "report_mysql_export.ndjson"
    captured: dict[str, object] = {}
    attempts = {"count": 0}
    backoffs: list[float] = []
    ...
    def fake_create_mysql_connection(**kwargs):
        captured["connect_kwargs"] = kwargs
        return FakeConnection()

    monkeypatch.setenv("LLM_DIFF_EXPORT_MYSQL_PASSWORD", "mysql-secret")
    monkeypatch.setattr("llm_behavior_diff.cli._sleep_export_retry", backoffs.append)
    monkeypatch.setattr(
        "llm_behavior_diff.cli._create_mysql_connection", fake_create_mysql_connection
    )
    ...
    connect_kwargs = captured["connect_kwargs"]
    assert isinstance(connect_kwargs, dict)
    assert connect_kwargs["host"] == "mysql.example.com"
    assert connect_kwargs["port"] == 3306
    assert connect_kwargs["database"] == "analytics"
    assert connect_kwargs["user"] == "svc_llm_diff"
    assert connect_kwargs["password"] == "mysql-secret"
    assert connect_kwargs["timeout_seconds"] == 6.0
...
def test_report_mysql_export_connector_requires_fields(tmp_path: Path, monkeypatch) -> None:
    report_path = tmp_path / "report_mysql_export_missing_fields.json"
    ndjson_path = tmp_path / "report_mysql_export_missing_fields.ndjson"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_mysql_export",
    ...
    report_path.write_text(json.dumps(report.model_dump(mode="json"), indent=2), encoding="utf-8")
    monkeypatch.delenv("LLM_DIFF_EXPORT_MYSQL_PASSWORD", raising=False)
    ...
    assert missing_host.exit_code == 1
    assert "export_mysql_host is required" in missing_host.output
    ...
    assert missing_password.exit_code == 1
    assert "MySQL password is required" in missing_password.output
    ...
    assert invalid_port.exit_code == 1
    assert "export_mysql_port must be > 0" in invalid_port.output


def test_report_mysql_export_connector_rejects_non_ndjson_format(tmp_path: Path) -> None:
    report_path = tmp_path / "report_mysql_export_non_ndjson.json"
    csv_path = tmp_path / "report_mysql_export_non_ndjson.csv"
    report = BehaviorReport(
        model_a="gpt-4o",
        model_b="gpt-4.5",
        suite_name="suite_mysql_export",
