...
def test_model_upgrade_workflow_has_factual_connector_inputs_and_export_wiring() -> None:
    ...
    assert job_env.get("EXPORT_MS_HOST") == "${{ vars.EXPORT_MS_HOST }}"
    assert job_env.get("EXPORT_MS_PORT") == "${{ vars.EXPORT_MS_PORT }}"
    assert job_env.get("EXPORT_MS_DATABASE") == "${{ vars.EXPORT_MS_DATABASE }}"
    ...
    assert job_env.get("EXPORT_MYSQL_HOST") == "${{ vars.EXPORT_MYSQL_HOST }}"
    assert job_env.get("EXPORT_MYSQL_PORT") == "${{ vars.EXPORT_MYSQL_PORT }}"
    ...
    assert job_env.get("EXPORT_MONGO_DATABASE") == "${{ vars.EXPORT_MONGO_DATABASE }}"
    assert job_env.get("EXPORT_MONGO_COLLECTION") == "${{ vars.EXPORT_MONGO_COLLECTION }}"
    ...
    assert job_env.get("LLM_DIFF_EXPORT_MONGO_URI") == "${{ secrets.MONGODB_URI }}"
    ...
    assert '--export-ms-host "$EXPORT_MS_HOST"' in run_script
    assert '--export-ms-port "${EXPORT_MS_PORT:-1433}"' in run_script
    assert '--export-ms-database "$EXPORT_MS_DATABASE"' in run_script
    ...
    assert '--export-mysql-host "$EXPORT_MYSQL_HOST"' in run_script
    assert '--export-mysql-port "${EXPORT_MYSQL_PORT:-3306}"' in run_script
    ...
    assert '--export-mongo-database "$EXPORT_MONGO_DATABASE"' in run_script
    assert '--export-mongo-collection "$EXPORT_MONGO_COLLECTION"' in run_script
    ...
    assert "MSSQL_PASSWORD secret is required when export_connector=mssql." in run_script
    assert (
        "EXPORT_MS_HOST repository variable is required when export_connector=mssql." in run_script
    )
    assert (
    ...
    assert (
        "EXPORT_MS_USER repository variable is required when export_connector=mssql." in run_script
    )
    assert (
    ...
    assert "MYSQL_PASSWORD secret is required when export_connector=mysql." in run_script
    assert (
    ...
    assert "MONGODB_URI secret is required when export_connector=mongodb." in run_script
    assert (
    ...
    assert 'elif [ "$EXPORT_CONNECTOR" = "mongodb" ]; then' in run_script
    ...
    assert "MONGODB_URI" in call_secrets
